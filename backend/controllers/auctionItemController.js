...
export const republishItem = catchAsyncErrors(async (req, res, next) => {
  ...
  const formattedStartTime = new Date(data.startTime).toLocaleString("en-IN", istOptions);

  return res.status(200).json({
    success: true,
    auctionItem,
    message: `Auction republished and will be active on ${formattedStartTime} (IST).`,
    createdBy,
  });
});
